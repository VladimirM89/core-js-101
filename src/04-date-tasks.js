...
function angleBetweenClockHands(date) {
  ...
  const hourAnglePerMinute = 360 / (12 * 60);

  const angle = Math.abs((30 * hour + hourAnglePerMinute * minutes) - minutes * minuteAngle);
  const angle2 = 360 - angle;

  return (Math.min(angle, angle2) * Math.PI) / 180;
}
